...
let findReport = name => {
    ...
    return new Promise((resolve, reject) => {
        let q = "SELECT Name, Description, DeveloperName, FolderName, LastRunDate FROM Report WHERE Name LIKE '%" + name + "%' LIMIT 5";
        org.query({query: q}, (err, resp) => {
            if (err) {
                reject("An error as occurred");
            } else {
...
let findContact = name => {
    ...
    return new Promise((resolve, reject) => {
        let q = "SELECT Id, Name, Phone, MobilePhone, Email, Birthdate FROM Contact WHERE Name LIKE '%" + name + "%' LIMIT 5";
        org.query({query: q}, (err, resp) => {
            if (err) {
                reject("An error as occurred");
            } else {
